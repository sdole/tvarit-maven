...
public class TvaritMojo extends AbstractMojo {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        final AWSOpsWorksClient awsOpsWorksClient = new AWSOpsWorksClient(awsCredentials);
        final AmazonRoute53Client awsRoute53Client = new AmazonRoute53Client(awsCredentials);
        ...
        waitTillInstanceOnline.waitTillInstanceOnline(awsOpsWorksClient,instanceId,getLog());
        warAppDeployer.deploy(project,getLog(),awsOpsWorksClient,instanceId);
        DomainNameMapper domainNameMapper = new DomainNameMapper();
        domainNameMapper.map(awsRoute53Client,infrastructureIds.getIpAddress());
        getLog().debug("Done!");
    }
